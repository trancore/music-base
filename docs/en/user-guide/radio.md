# Internet radio

Open the “Radio” screen to add and play internet radio stations. Enter a station name and its direct audio stream URL, then select “Test and save”.

Use the search icon to search Radio Browser. Results are limited to stations Radio Browser currently considers reachable, and the app uses its resolved direct stream URL rather than a playlist or redirect URL. Select a result to test, save, and start playback immediately.

An audio stream URL delivers audio directly. It is different from the station’s web page URL. For example, this HappyHardcore.com page is not a stream URL:

```text
https://www.happyhardcore.com/radio/player/radio.asp
```

An example of a direct stream URL used by the page is:

```text
https://u1.happyhardcore.com/
```

Saved stations can be played, edited, and deleted. Stream URLs can change when a station changes providers, so update the saved station if playback stops working. Station data is saved in the app settings on this device.

Use the “Manual order”, “Name (A–Z)”, “Name (Z–A)”, or “Genre” sort options. In Manual order, drag a station using the handle on its left side. The manual order is saved on this device.
