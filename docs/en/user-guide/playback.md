...
Choose “Import M3U playlist” to read a UTF-8 `.m3u` or `.m3u8` file. You can choose the top level or an existing folder as its destination. Extended M3U files containing `#EXTM3U` and `#EXTINF`, absolute paths, and paths relative to the playlist file are supported. Before importing, the app shows exact matches and unavailable tracks. If the M3U source root differs from the current library, you can select a root mapping that has been verified against tracks in the library cache. Imported order is preserved, and unavailable paths remain in the playlist so they can recover after switching or rescanning the source. The filename becomes the initial playlist name. Importing does not move or modify audio files.
