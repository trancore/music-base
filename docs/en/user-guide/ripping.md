...
The import plan lets you review FLAC or MP3, the destination, track order, and tag candidates. Selecting a MusicBrainz release saves its track title, artist, album, and release date as tags. If existing files are found, the import stops before starting and lists them; files are never overwritten. CD drive detection and actual encoding are Windows-specific features.
